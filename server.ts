...
const authenticateToken = (req: AuthenticatedRequest, res: Response, next: NextFunction): void => {
  ...
  try {
    // Simulated token: 'user-token-ROLE-id'
...
app.post("/api/auth/login", (req: Request, res: Response): void => {
  ...
  if (!user) {
    res.status(401).json({ error: "Invalid username, email, or account inactive" });
    return;
  }
  ...
  if (user.isVerified === false) {
    res.status(403).json({
      ...
      email: user.email,
      role: user.role,
      _debugCode: user.verificationCode // Expose code under debug key so test users can see it immediately
    });
    return;
  }

  // Create simulated token: user-token-ROLE-USERID-TIMESTAMP
  const token = `user-token-${user.role}-${user.id}-${Date.now()}`;
...
app.post("/api/payments/fee", authenticateToken, authorizeRole(['ADMIN']), (req: AuthenticatedRequest, res: Response): void => {
  ...
  try {
    const payment = db.processFeePayment(parseInt(studentId, 10), {
...
app.post("/api/auth/signup", (req: Request, res: Response): void => {
  ...
  if (!username || !email || !password || !firstName || !lastName || !classId) {
    res.status(400).json({ error: "Missing required student sign-up fields" });
    return;
  }
  ...
  try {
    // 1. Create accompanying user record
    ...
    const nextStudentId = db.getState().nextId.students;
    const studentIdxStr = `STU${String(nextStudentId).padStart(3, '0')}`;
    const student: any = {
      id: nextStudentId,
      userId: user.id,
      studentId: studentIdxStr,
      firstName,
      lastName,
    ...
    db.getState().students.push(student);
    db.getState().nextId.students = nextStudentId + 1;

    // 3. Enroll student in chosen academic class cohort.
    const nextEnrollId = db.getState().nextId.studentEnrollments;
    const enrollment: any = {
      id: nextEnrollId,
      studentId: student.id,
      classId: parseInt(String(classId), 10),
      enrollmentDate: student.enrollmentDate,
      status: 'PENDING'
    };
    db.getState().studentEnrollments.push(enrollment);
    db.getState().nextId.studentEnrollments = nextEnrollId + 1;

    // Save changes
    (db as any).save();

    db.log(user.id, user.username, user.role, 'AUTH_SIGNUP', `Student self-onboarded. Verification code generated: ${verificationCode}`);
    ...
    res.status(201).json({
      ...
      email: user.email,
      role: user.role,
      _debugCode: verificationCode // Expose code under debug key so test users can see it in sandbox UI immediately
    });
  } catch (err: any) {
...
app.post("/api/auth/signup-staff", (req: Request, res: Response): void => {
  ...
  if (!username || !email || !password || !firstName || !lastName) {
    res.status(400).json({ error: "Missing required faculty sign-up fields" });
    return;
  }
  ...
  try {
    ...
    db.log(user.id, user.username, user.role, 'AUTH_SIGNUP_STAFF', `Lecturer self-onboarded. Verification code generated: ${verificationCode}`);

    // Send E-mail verification
    sendEmail({
      to: email,
      subject: "Verify Your University Hub Faculty Account",
      text: `Hello ${firstName},\n\nWelcome to University Hub Faculty! Your 6-digit verification code is: ${verificationCode}\n\nPlease enter this code in the app to activate your account.\n\nBest regards,\nUniversity Hub Faculty Board`,
      html: `
        <div style="font-family: sans-serif; max-width: 500px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px; background: #ffffff;">
    ...
    res.status(201).json({
      ...
      email: user.email,
      role: user.role,
      _debugCode: verificationCode // Expose code under debug key so test users can see it immediately
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
app.post("/api/auth/forgot-password-request", (req: Request, res: Response): void => {
  ...
  if (!username || !email) {
    res.status(400).json({ error: "Username and registered email address are required" });
    return;
  }
...
app.post("/api/auth/forgot-password", (req: Request, res: Response): void => {
  ...
  if (!username || !email || !code || !newPassword) {
    res.status(400).json({ error: "Missing required fields (username, email, verification code, new password)" });
    return;
  }
